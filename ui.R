...
dashboardPage(
  ...
  dashboardSidebar(#       sliderInput("count","",min=1,max=1000,value=c(1,1000),step=10, ticks=FALSE),
    #       uiOutput("industries"),
  ...
  dashboardBody(tabItems(tabItem(
    ...
    fluidRow(box(
      ...
      div(style = "display:inline-block",selectInput(
        "player","Player",c("All",playerChoice), width = 180
      )),
      div(style = "display:inline-block",selectInput(
        "team","Team",c("All",teamChoice), width = 180
      )),
      div(style = "display:inline-block",selectInput(
        "country","Country",c("All",countryChoice), width = 180
      )),
      div(style = "display:inline-block",selectInput("tier","Tier",c("All",1,2,3,4), width = 80)),
      div(style = "display:inline-block",selectInput(
        "position","Position",c("All","GK","DF","MF","FW"), width = 80
      )),
      # div(style="display:inline-block",sliderInput("year","Year(s)",min=1973,max=2014,value=c(1992,2014),sep="", width=200))
      sliderInput(
        "year","Year(s)",min = 1973,max = 2014,value = c(1992,2014),sep = "", width =
          200
      )
    )),
    fluidRow(column(
